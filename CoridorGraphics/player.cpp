...
void GameLogic::output(int pole[17][17])
{
...
bool GameLogic::inPole(int x, int y)
{
	return x >= 0 && x < realSize && y >= 0 && y < realSize;
}

pair<int, int> GameLogic::inPole(pair<int, int> p)
{
...
pair<int, int> GameLogic::step(int x, int y, char ch)
{
	switch (ch)
	{
	case 'l':
...
bool GameLogic::isPlayer(int new_x, int new_y)
{
	for (int i = 0; i < 2; i++)
	{
...
int GameLogic::move(int pole[17][17], int player_id, int new_x, int new_y)
{
...
int GameLogic::placeWall(int pole[17][17], int player_id, int x, int y, char dir)
{
...
bool GameLogic::isWall(int x0, int y0, int x1, int y1)
{
	return pole[(x0 + x1) / 2][(y0 + y1) / 2] == w;
}

bool GameLogic::aZvezda(int x, int y, int p)
{
	bool field[9][9];
...
bool GameLogic::checkDistanse(int new_x, int new_y, int player_id)
{
...
bool GameLogic::jumpCheck(int new_x, int new_y, int player_id)
{
