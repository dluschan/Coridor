...
{
    bool flag = true;
    pair<int, int> coords = step(x, y, ans, 2);
    int new_x1 = coords.first;
    int new_y1 = coords.second;
    coords = step(x, y, ans, 4);
    int new_x2 = coords.first;
    int new_y2 = coords.second;
    if (!(new_x2 < 0 || new_x2 > 17 || isWall(new_x1, new_y1, new_x2, new_y2, pole)))
    {
        pole[new_x2][new_y2] = p;
        pole[x][y] = -1;
        x = new_x2;
        y = new_y2;
        flag = false;
    ...
    else
    {
        ...
        case 'r':
        {
            if ((new_y1 > 15 || isWall(new_x1, new_y1, new_x1, new_y1 + 2, pole)) && (new_y1 < 2 || isWall(new_x1, new_y1, new_x1, new_y1 - 2, pole)))
            {
                cout << "Error: you cant jump here (" << ans << ")!" << endl;
                return !flag;
            }
            while (flag)
            {
                cout << "Chose direction - down or up (d/u): ";
                char tmp;
                cin >> tmp;
                cout << endl;
                switch (tmp)
                ...
                case 'u':
                {
                    coords = step(new_x1, new_y1, tmp, 2);
                    new_x2 = coords.first;
                    new_y2 = coords.second;
                    pole[new_x2][new_y2] = p;
                    pole[x][y] = -1;
                    x = new_x2;
                    y = new_y2;
        ...
        case 'u':
        {
            if ((new_x1 > 15 || isWall(new_x1, new_y1, new_x1 + 2, new_y1, pole)) && (new_x1 < 2 || isWall(new_x1, new_y1, new_x1 - 2, new_y1, pole)))
            {
                cout << "Error: you cant jump here (" << ans << ")!" << endl;
                return !flag;
            }
            while (flag)
            {
                cout << "Chose direction - left or right (l/r): ";
                char tmp;
                cin >> tmp;
                cout << endl;
                switch (tmp)
